...
{
    public abstract class ShiNengShiHuiTestBase : AbpIntegratedTestBase<ShiNengShiHuiTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ShiNengShiHuiTestBase()
        {
        ...
        protected void UsingDbContext(Action<ShiNengShiHuiDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ShiNengShiHuiDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ShiNengShiHuiDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ShiNengShiHuiDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ShiNengShiHuiDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ShiNengShiHuiDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ShiNengShiHuiDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ShiNengShiHuiDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ShiNengShiHuiDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ShiNengShiHuiDbContext, Task<T>> func)
        {
            T result;
